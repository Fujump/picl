...
def main(args):
    ...
    sub_labels=['政治错误', '偏见歧视', '违法犯罪', '脏话侮辱', '道德伦理', '身体伤害', '财产隐私', '心理健康', '淫秽色情', '变体词']
    for sub_label in sub_labels:
        ...
        for seed in [100,200,300]:
            ...
            all_data_total=pd.read_json("/data/home/huq/deepai/dataset/all_data_total.json",lines=True)
            test_dataset = test_dataset.merge(all_data_total[['text', 'label_sub']], on='text', how='left')
            test_dataset=test_dataset[test_dataset['label_sub']==sub_label]
            # print(test_dataset.head())
            neg_ind=np.load(f"/data/home/huq/deepai/dataset/test_idxs_neg_sample_{seed}.npy")
            negs=all_data_total.iloc[neg_ind]
            negs['label_total']=0
            negs = negs.rename(columns={'label_total': 'label'})
            test_dataset = pd.concat([test_dataset, negs], ignore_index=True)
            replace_str = "政治敏感、淫秽色情、偏见歧视、脏话辱骂、违法犯罪、伤害身体、财产隐私、道德伦理、违法犯罪、变体词"
            test_dataset['text'] = test_dataset.apply(lambda row: row['text'].replace(replace_str, sub_label), axis=1)
            ...
            dat.to_csv('/data/home/huq/deepai/output_gen_sub/{model}_{ranking}_{ice_num}_{knn_num}_{task}_{dpp_candidate_num}_{test_retrieving}_{noise_retrieving}_{tau}_{sub_label}_{seed}.csv'.format(model=args.pretrained_model_name ,ranking=args.ranking, tau=args.tau, dpp_candidate_num=args.dpp_candidate_num, ice_num=args.ice_num, task=args.task, test_retrieving=args.test_retrieving,  noise_retrieving=args.noise_retrieving, sub_label=sub_label,seed=seed,knn_num=args.knn_num))
        ...
        resul.to_csv('/data/home/huq/deepai/output_gen_sub/result_{model}_{ranking}_{ice_num}_{knn_num}_{task}_{dpp_candidate_num}_{test_retrieving}_{noise_retrieving}_{tau}_{sub_label}_{seed}.csv'.format(model=args.pretrained_model_name ,ranking=args.ranking, tau=args.tau, dpp_candidate_num=args.dpp_candidate_num, ice_num=args.ice_num, task=args.task, test_retrieving=args.test_retrieving,  noise_retrieving=args.noise_retrieving, sub_label=sub_label,seed=seed,knn_num=args.knn_num))
